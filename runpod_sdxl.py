...
app.add_middleware(
    ...
    allow_methods=["GET", "POST", "OPTIONS"],  # Include "OPTIONS" to allow preflight requests.
    allow_headers=["*"],  # You can specify the allowed headers here.
    allow_credentials=True,  # Adjust this as needed.
)
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--prompt", type=str, default="boy on a horse, in flt1vctr style --n (worst quality:1.5), (low quality:1.5), (normal quality:1.5), lowres, bad anatomy, bad hands, multiple eyebrow, (cropped), extra limb, missing limbs, deformed hands, long neck, long body, (bad hands), signature, username, artist name, conjoined fingers, deformed fingers, ugly eyes, imperfect eyes, skewed eyes, unnatural face, unnatural body, error, painting by bad-artist", help="prompt / プロンプト")
    parser.add_argument(
    ...
    parser.add_argument("--strength", type=float, default=1, help="img2img strength / img2img時のstrength")
    parser.add_argument("--images_per_prompt", type=int, default=4, help="number of images per prompt / プロンプトあたりの出力枚数")
    ...
    parser.add_argument("--H", type=int, default=768, help="image height, in pixel space / 生成画像高さ")
    parser.add_argument("--W", type=int, default=768, help="image width, in pixel space / 生成画像幅")
    parser.add_argument(
    ...
    parser.add_argument("--steps", type=int, default=25, help="number of ddim sampling steps / サンプリングステップ数")
    parser.add_argument(
        "--sampler",
        type=str,
        default="euler_a",
...
if args.network_module:
    ...
    for i, network_module in enumerate(args.network_module):
        ...
        network_mul = 0.7 if args.network_mul is None or len(args.network_mul) <= i else args.network_mul[i]
...
def main(args):
    ...
    if args.sampler == "euler_a" or args.sampler == "k_euler_a":
        scheduler_cls = EulerAncestralDiscreteScheduler
        scheduler_module = diffusers.schedulers.scheduling_euler_ancestral_discrete
        has_clip_sample = False
...
# Function to remove existing images in the folder
def remove_existing_images(folder):
...
async def upscale(request: UpscaleRequest):
    ...
    image_input_folder = 'upscale_input'
    remove_existing_images(image_input_folder)

    # URL of the image you want to download
    image_url = request.image_url

    # Save the image from the URL to the folder
    save_image_from_url(image_url, image_input_folder)
    # Decode base64 string to image bytes
    image_bytes = base64.b64decode(request.image_base64)
    # Save image to disk
    image_path = f"upscale_input/input.png"
    ...
    args.W = request.width
    args.H = request.height
    args.image_path = image_path
    args.steps = 40
    args.strength = request.strength
    args.outdir = request.outdir
    args.images_per_prompt = request.num
    args.scale = request.cfg
    main(args)
...
@app.post("/img2img")
async def generateFromImages(request: ImageRequest):
    # Remove existing images from the folder
    image_input_folder = 'image_input'
    remove_existing_images(image_input_folder)
    ...
    image_bytes = base64.b64decode(request.image_base64)
    # Save image to disk
    image_path = f"/workspace/img2img/input.png"
    with open(image_path, "wb") as f:
        f.write(image_bytes)
    parser = setup_parser()
    args = parser.parse_args()
    ...
    for file in os.listdir('outputs'):
        if file.endswith('.png'): 
            with open(f'outputs/{file}', 'rb') as f:
                ...
                img_str = base64.b64encode(img_bytes).decode("utf-8")
                job_output.append(img_str)

    return {"response": job_output}
    
# @app.post("/generate")  
# async def generate_images(request: TextRequest):
#     parser = setup_parser()
#     args = parser.parse_args()
#     load_lora(args)
...
# @app.post("/generate")
# async def generate_and_stream_images(request: TextRequest):
#     return StreamingResponse(generate_images(request), media_type="text/plain")

# @app.post("/generate")
def generate(prompt, width=768, height=768, num = 4, cfg = 7):
    parser = setup_parser()
    args = parser.parse_args()
    load_lora(args)
    job_output = []
...
'''
    ...
    response = generate(job_input["prompt"])
    return response
